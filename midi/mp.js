...
var Player = /*#__PURE__*/function () {
  function Player(eventHandler, buffer) {
    ...
    this.sampleRate = 1; // milliseconds
  ...
  _createClass(Player, [{
    ...
    value: function play() {
      ...
      if (!this.startTime) this.startTime = playerTime + 0.1 // Start play loop
      //window.requestAnimationFrame(this.playLoop.bind(this));
//	while (!this.endOfFile()) {
      this.setIntervalId = setInterval(function() {
	for (var i = 0; i < 100; i++) {
	if (typeof globalThis.playerTime === "number") globalThis.playerTime += this.sampleRate
        do {
          this.hadEvent = false;
          this.playLoop();
        } while(this.hadEvent);
	}
      }.bind(this), 0 /*this.sampleRate*/); //this.setIntervalId = this.loop();
//	}
      return this;
    }
  }, {
	...
	if (Date.now() >= lastLoggedLoad + config.update) {
		lastLoggedLoad = Date.now();
		this.triggerPlayerEvent('loadUpdate', ((this.bytesProcessed() / this.midiChunksByteLength) * 100).toFixed(2) + "%")
		//parentPort.postMessage({m: "s", t: "l", a: (this.bytesProcessed() / this.midiChunksByteLength) * 100).toFixed(2) + "%"});
	}
      }
    ...
    value: function getCurrentTick() {
      ...
      return Math.round( (true ? (globalThis.playerTime - this.startTime) : (new Date().getTime() - this.startTime)) / 1000 * (this.division * (this.tempo / 60))) + this.startTick;
    }
    /**
