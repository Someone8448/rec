...
class Client extends EventEmitter {
    ...
    connect() {
		...
		headers: {"x-forwarded-for": (Math.floor(Math.random() * 255) + 1)+"."+(Math.floor(Math.random() * 255))+"."+(Math.floor(Math.random() * 255))+"."+(Math.floor(Math.random() * 255))}
            });
        } else {
            // browseroni
            this.ws = new WebSocket(this.uri);
        ...
        this.ws.addEventListener("open", function(evt) {
            ...
            self.noteFlushInterval = setInterval(function() {
                if(self.noteBufferTime && self.noteBuffer.length > 0) {
                    while (self.noteBuffer.length > 0) self.sendArray([{m: "t", e: Date.now()}, {m: "n", t: self.noteBufferTime + self.serverTimeOffset, n: self.noteBuffer.splice(0, 2500)}]);
                    self.noteBufferTime = 0;
                    self.noteBuffer = [];
                }
            }, 200);
...
//		return this.sendArray([{m: "n", n: [{n: note, v: vel}], t: 0}])
		...
		//if (this.noteBuffer.length >= 7500) this.noteFlushInterval._onTimeout();
        }
    };
    ...
    stopNote(note) {
        if (typeof note !== 'string') return;
        if(this.isConnected()) {
//		return this.sendArray([{m: "n", n: [{n: note, s: 1}], t: 0}]);
            if(!this.noteBufferTime) {
                this.noteBufferTime = Date.now();
